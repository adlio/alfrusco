...
fn test_background_job_fresh() {
    ...
    let file = std::fs::File::options().write(true).open(&last_run_file).unwrap();
...
fn test_background_job_error() {
    ...
    let mut cmd = Command::new("non_existent_command");
...
fn create_test_workflow(temp_path: &PathBuf) -> Workflow {
